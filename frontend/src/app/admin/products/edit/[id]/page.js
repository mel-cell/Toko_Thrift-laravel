...
const formSchema = z.object({
  ...
  gambar: z.any()
    .optional()
    .refine((file) => {
      if (!file || file.length === 0) return true; // No file selected, valid
      return file?.[0]?.size <= 5000000; // File selected, check size
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (productData) {
      ...
      form.reset({
        ...
        harga: product.harga,
        stok: product.stok,
        kategori_pakaian_id: product.kategori_pakaian_id,
        gambar: undefined, // Image input should be reset
      });
    }
  }, [productData, form]);
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      const response = await fetcher.post(`/api/admin/pakaians/${id}`, formData, {
        headers: {
  ...
  if (productLoading || categoriesLoading) return <div>Loading product details...</div>;
  if (productError) return <div>Failed to load product: {productError.message}</div>;
  if (categoriesError) return <div>Failed to load categories: {categoriesError.message}</div>;
  if (!productData || !categoriesData) return null; // Should not happen if loading states are handled
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field: { value, onChange, ...fieldProps } }) => (
              <FormItem>
                <FormLabel>Product Image (leave blank to keep current)</FormLabel>
                <FormControl>
                  <Input
                    {...fieldProps}
                    type="file"
